...
func main() {
	...
	// 添加字体
	dc.SetRGB(0, 0, 0)
	if err := dc.LoadFontFace("./tiff-demo/demo09/JingDianZongYiTiJian-1.ttf", 300); err != nil {
		panic(err)
	}
	dc.DrawStringAnchored("泉工-C02D001", 100, 343, 0, 0.5)

	// 添加车辆信息二维码
	im, err := gg.LoadImage("./tiff-demo/demo09/img.png")
	if err != nil {
		log.Fatal(err)
	}
	...
	img := dc.Image()
	f, _ := os.OpenFile("./tiff-demo/demo09/out.tiff", os.O_CREATE|os.O_RDWR, 0666)
